...
func (m *MockObjectStore) SaveFileToBucket(bucket string, filePath string, sha3_384_encoded string, name string, version string, summary string, description string, confinement string, base string, grade string, architectures, refreshControl []string, plugs, slots map[string]map[string]interface{}) (*models.Metadata, error) {
