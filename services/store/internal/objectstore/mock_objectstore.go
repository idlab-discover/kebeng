...
func (m *MockObjectStore) SaveFileToBucket(bucket string, filePath string, sha3_384_encoded string, name string, version string, summary string, description string, confinement string, base string, grade string, architectures []string, plugs, slots map[string]map[string]interface{}) (*models.Metadata, error) {
	args := m.Called(bucket, filePath, sha3_384_encoded, name, version, summary, description, confinement, base, grade, architectures, plugs)
	if args.Get(0) != nil {
