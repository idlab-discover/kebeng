...
func (m *MockStoreClient) AddRevision(snapName string, sha3384 string, size uint64, architectures []string, tracksAndChannels []string, unscannedFileName string, minioFilePath string) *proto.AddRevisionResponse {
	args := m.Called(snapName, sha3384, size, architectures, tracksAndChannels, unscannedFileName, minioFilePath)
